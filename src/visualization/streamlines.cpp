...
// Wind colormap: blue (slow) -> cyan -> green -> yellow -> orange (fast)
// Matches CFD reference: slow=blue, accelerated=yellow/orange
vec3 windColor(float t) {
    t = clamp(t, 0.0, 1.0);
    vec3 c0 = vec3(0.10, 0.25, 0.75);  // dark blue  (stagnation / slow)
    vec3 c1 = vec3(0.15, 0.65, 0.90);  // cyan       (free-stream)
    vec3 c2 = vec3(0.20, 0.85, 0.45);  // green      (mild acceleration)
    vec3 c3 = vec3(0.95, 0.90, 0.10);  // yellow     (fast, over surfaces)
    vec3 c4 = vec3(1.00, 0.40, 0.05);  // orange-red (very fast / edge)
...
void main() {
    vec3 col = windColor(vT);
    float a = vAlpha * 0.88;  // uniform opacity — all lines equally visible
    ...
    int numLines, int maxSteps, WindDirection windDir)
{
    ...
    // Seed further from the model so wind visually starts from a distance
    switch (windDir) {
    ...
    auto isValid = [&](glm::vec3 p) -> bool {
        if (p.x < 0 || p.x >= nx - 1 || p.y < 1 || p.y >= ny - 1 || p.z < 1 || p.z >= nz - 1)
            return false;
    ...
    for (int i1 = 0; i1 < sqrtN && generated < numLines; i1++) {
        for (int i2 = 0; i2 < sqrtN && generated < numLines; i2++) {
            ...
            glm::vec3 pos(0.0f);
            // Build seed position: fixed axis gets seedSlice, others get v1/v2
            float coords[3];
            ...
            for (int step = 0; step < maxSteps; step++) {
                ...
                // Stagnant / not-yet-developed region: coast in wind direction
                // without emitting a vertex. Lines reach the aircraft even
                // before the LBM front has propagated across the full domain.
                if (mag < 1e-4f) {
                    pos += coastDir;
                    continue;
                }

                // Arc-length RK4: normalize velocity so each step = ds grid cells.
                // This keeps curves smooth at any velocity magnitude.
                float arcStep = ds / mag;
                glm::vec3 k1 = arcStep * vel;
                glm::vec3 p2 = pos + 0.5f * k1;
                if (!isValid(p2)) { pos += k1; continue; }
            ...
            if (lineVerts.size() >= 3) {
                // Set alpha: fade in first 8%, full opacity in middle, fade out last 20%
                int len = (int)lineVerts.size();
                for (int i = 0; i < len; i++) {
                    float t = (float)i / (float)(len - 1);
                    float fadeIn  = std::min(1.0f, t / 0.08f);
                    float fadeOut = std::min(1.0f, (1.0f - t) / 0.20f);
...
void StreamlineRenderer::render(const glm::mat4& mvp, float maxVel) {
    if (vertexCount == 0 || lineCount == 0) return;

    glEnable(GL_DEPTH_TEST);
    glDepthMask(GL_FALSE);  // Don't write to depth buffer (transparent)
    glEnable(GL_BLEND);
    // Standard alpha blending: preserves actual colors (blue/cyan/yellow/orange)
